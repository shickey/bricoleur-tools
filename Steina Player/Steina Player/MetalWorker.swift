...
class MetalView : NSView {
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        mouseLocation.y = 480 - mouseLocation.y
        if let d = delegate {
            d.metalViewBeganTouch(self, location: mouseLocation)
        }
    }
    ...
    override func mouseDragged(with event: NSEvent) {
        ...
        mouseLocation.y = 480 - mouseLocation.y
        if let d = delegate {
            d.metalViewMovedTouch(self, location: mouseLocation)
        }
    }
    ...
    override func mouseUp(with event: NSEvent) {
        ...
        mouseLocation.y = 480 - mouseLocation.y
        if let d = delegate {
            d.metalViewEndedTouch(self, location: mouseLocation)
        }
    }
...
var publicDepthBuffer : MTLBuffer! = nil
...
func initMetal(_ hostLayer: CAMetalLayer) {
    ...
    // Set up public depth buffer for reading
    publicDepthBuffer = device.makeBuffer(length: 4 * 640 * 640, options: .storageModeShared)
...
func render(_ numEntities: Int) {
    ...
    if let drawable = metalLayer.nextDrawable() {
        ...
        let publicDepthBlitCommandEncoder = commandBuffer.makeBlitCommandEncoder()!
        let depth = pass.depthAttachment.texture!
        publicDepthBlitCommandEncoder.copy(from: depth, 
                                           sourceSlice: 0, 
                                           sourceLevel: 0, 
                                           sourceOrigin: MTLOriginMake(0, 0, 0), 
                                           sourceSize: MTLSizeMake(depth.width, depth.height, 1), 
                                           to: publicDepthBuffer,
                                           destinationOffset: 0, 
                                           destinationBytesPerRow: 4 * depth.width, 
                                           destinationBytesPerImage: 4 * depth.width * depth.height,
                                           options: MTLBlitOption.depthFromDepthStencil)
        publicDepthBlitCommandEncoder.endEncoding()
